...
const StatusOrdersTable = ({
  ...
  const statusList = ["accepted", "onProgress", "completed", "paid"];
  return (
          ...
          {list?.map((dat) => (
            <tr className="bg-white border-b  hover:bg-gray-50 ">
              ...
              <td className="px-6 py-4 text-right">
                <div className="font-sm  text-blue-600  hover:underline">
                  <select
                    id="countries"
                    value={dat.status}
                    ...
                    onChange={(e) => trigger(dat._id, e.target.value)}
                  >
                    {statusList
                      .filter(
                        (stats, index) =>
                          index >= statusList.indexOf(dat.status)
                      )
                      .map((dat) => (
                        <option
                          value={dat}
              ...
              <td className="px-6 py-4">
                {dat.status === "accepted" &&
                  <div
                    ...
                    onClick={() => revokeOrder(dat._id)}
                  >
                    <XIcon color="red" className="h-[30px] cursor-pointer" />
                  </div>
                }
              </td>
            </tr>
...
export default StatusOrdersTable;
