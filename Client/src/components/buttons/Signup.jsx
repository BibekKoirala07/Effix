...
import { createUser, getService } from "../../services/user.services";
...
const Signup = ({ onClose }) => {
  ...
  const handleSubmit = async () => {
    if (data.password === data.password2) {
      ...
      const [response, error] = await createUser(payload);
      if (response) {
  ...
  return (
    <div className="h-[100vh] overflow-y-scroll fixed top-[0%] left-[0%] w-[100vw]  z-[122200] ">
      <div className="z-[122200] flex flex-col justify-center items-center ">
        <div
          onClick={() => backdrop()}
          className="w-full h-full fixed top-0 left-0 backdrop-blur-sm bg-black/50 z-[1]"
        ></div>

        <div className="min-w-screen w-max z-[12112]  flex items-center justify-center px-5 py-5">
          <div className="bg-gray-100 max-w-[1080px] text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden">
            <div className="md:flex w-full">
              <div className="hidden md:block w-1/2 bg-accent py-10 px-10">
                <svg
                ...
                <div >
                  <div>
                    ...
                    <input
                      ...
                      id="first_name"
                      value={data.name}
                      className="bg-gray-50 mb-3 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 "
                      placeholder="John"
                      required
                      onChange={(e) => handleInput(e, "name")}
                    />
                  </div>
                  <div className="grid gap-1 mb-1 lg:grid-cols-2">
                    <div>
                      <label
                        for="phone"
                        className="block mb-2 text-sm font-medium text-gray-900 "
