...
const Email = (props) => {
  ...
  const handleLogin = async () => {
    const [response, error] = await updateEmail(user.user._id, {
      email: data.email,
      password: data.password,
    });
    if (response) {
      dispatch(toogleEmail(false));
      toast.success("Email Change Request sent to your mail.");
    } else {
      console.log(error);
      ...
      <div className="bg-white pt-5 md:w-full md:max-w-md shadow w-full rounded-[20px] z-[22] flex flex-col justify-center items-center">
        <div className="px-7 py-7 w-[80%]">
          ...
          <button
            ...
            onClick={handleLogin}
          >
            <span className="inline-block mr-2">Change Email</span>
            <svg
